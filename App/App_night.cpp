...
int main(int argc, char*argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(1280, 720, "Night Track", NULL, NULL);
    if (window == NULL)
    {
    ...
    // Light variables
    glm::vec3 moonDir   = glm::normalize(glm::vec3(-0.5f, -1.0f, -0.3f));
    glm::vec3 moonColor = glm::vec3(0.6f, 0.7f, 1.0f);
...
// ----------------------------------------------------
    ...
    glUniform3f(glGetUniformLocation(texturedShaderProgram, "sunDir"), moonDir.x, moonDir.y, moonDir.z);
    glUniform3f(glGetUniformLocation(texturedShaderProgram, "sunColor"), moonColor.x, moonColor.y, moonColor.z);
    ...
    ModelData moonData = createSphere(1.0f);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Animate moon direction over time
        moonDir = glm::normalize(glm::vec3(sin(glfwGetTime()*0.1f), -1.0f, cos(glfwGetTime()*0.1f)));
        ...
        glm::vec3 shadowDir = headlightsOn ? hlDir : moonDir;
        ...
        glm::vec3 moonPos = -moonDir * 100.0f;
        glm::mat4 moonModel = glm::translate(glm::mat4(1.0f), moonPos) *
                             glm::scale(glm::mat4(1.0f), glm::vec3(5.0f));
        glUniformMatrix4fv(glGetUniformLocation(lightShaderProgram, "model"), 1, GL_FALSE, glm::value_ptr(moonModel));
         glUniform4f(glGetUniformLocation(lightShaderProgram, "lightColor"), 0.6f, 0.7f, 1.0f, 1.0f);
        glBindVertexArray(moonData.VAO);
        glDrawElements(GL_TRIANGLES, moonData.indexCount, GL_UNSIGNED_INT, 0);
        ...
        // Bind the pole texture to texture unit 0
            ...
            for (const auto& lampPos : lampPositions) {
                ...
                glm::vec3 poleLightDir = headlightsOn ? glm::normalize(polePosition - hlPos) : moonDir;
        ...
        for (size_t i = 0; i < grandstandPositions.size(); ++i) {
            ...
            glm::mat4 grandstandModel = glm::translate(glm::mat4(1.0f), grandstandPositions[i]) *
             ...
             glm::vec3 gsLightDir = headlightsOn ? glm::normalize(grandstandPositions[i] - hlPos) : moonDir;
